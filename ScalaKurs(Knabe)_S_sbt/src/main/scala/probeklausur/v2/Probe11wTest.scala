...
//        list.map(_.length).foldLeft(0)((accu, elem) => accu + elem)
        list.foldLeft(0)((accu, elem) => accu + elem.length)
...
//        list.foldLeft(0)((accu, elem) => if (elem.length() > 0 && elem.charAt(0).isUpper) accu + 1 else accu)
...
//        list.map(_.filter(accept).length).foldLeft(0)((accu, elem) => accu + elem)
...
//      /* Versteckte Implementierung: */ = _filter(list, accept)
        list.foldLeft(List[String]())((accu, elem) => if (accept(elem)) elem :: accu else accu).reverse
...
//	    /* Versteckte Implementierung: */ = _merge(aList, bList)
        ...
        while (!a.isEmpty && !b.isEmpty) {
          merged = b.head :: a.head :: merged
          a = a.tail
          b = b.tail
        }
        var rest = if (a.isEmpty) b else a
//        rest.foldLeft(merged)((accu, elem) => elem :: accu).reverse
...
//    	/* Versteckte Implementierung: */ _stringize(expr)
      ...
      case Mul(a, b) => {
        def paren(x: Expr) = x match {
          case Add(_, _) => "(" + stringize(x) + ")"
          case _ => stringize(x)
        }
        paren(a) + "*" + paren(b)
      }
